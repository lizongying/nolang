	...
	for _, name := range sortedNames {
		...
		for _, label := range g.curCFG.Order {
			...
			if !first {
				...
				// When hasBranchMove is true and a runtime bitmap is available,
			// the CFG may be incomplete (e.g., option match arms where move
			// happens inside an if-branch). In that case, do NOT use the
			// AssignedFact direct-free shortcut — fall through to the runtime
			// bitmap check instead, which accurately tracks move state.
			if tri == triMustNot && !aFirst && !(g.hasBranchMove && g.movedBitmapBase != "") {
					...
					if n, ok := g.funcNumResults[lookupName]; ok && n >= 1 {
						if g.funcResultLLVMType != nil {
							if ts, ok2 := g.funcResultLLVMType[lookupName]; ok2 && len(ts) >= 1 {
								retType := ts[0]
								if os.Getenv("NOLANG_DEBUG_IT") != "" {
									fmt.Fprintf(os.Stderr, "[debug-it]   funcResultLLVMType[%q]=%q (n=%d) returning\n", lookupName, retType, n)
								}
								if retType == "i1" {
									retType = "i64"
								}
					...
					if nestedFields != nil {
						...
						for _, nsf := range nestedSL.Fields {
							if nfIdx, ok := nestedFieldIdxMap[nsf.Name]; ok {
								...
								// 但對來自參數/局部堆變數的 str/vec/arr 值需要深層 clone，
								// 否則結構體欄位與源變數共享同一 data 指標，
								// 函數退出時 emitStructFieldsFree 和 emitHeapFree 會 double-free。
								if _, isStrLit := nsf.Value.(*parser.StringLiteral); isStrLit {
									...
									// Check if the value is an Identifier pointing to a
									// heap-owning local/param; if so, deep-clone str/vec/arr
									// to avoid double-free on function exit.
									needClone := false
									if ident, ok := nsf.Value.(*parser.Identifier); ok {
										if g.funcParams != nil && g.funcParams[ident.Value] {
											needClone = true
										}
										if g.heapVars != nil {
									...
									irFieldType := toLLVMType(nfType)
									if needClone && (irFieldType == "%str-long" || irFieldType == "%vec" || irFieldType == "%arr") {
										srcIdent := nsf.Value.(*parser.Identifier)
										dataFieldIdx := 2
										if irFieldType == "%arr" {
											dataFieldIdx = 1
										}
										g.emitContainerClone(sb, g.varAddr(srcIdent.Value), nfGEP, irFieldType, dataFieldIdx, "")
									} else {
										nfVal := g.generateExprWithSB(sb, nsf.Value)
										nfVal = g.stripLLVMType(nfVal)
