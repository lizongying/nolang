...
func (g *Generator) generateCastExpression(sb *strings.Builder, e *parser.CastExpression) string {
	...
	if srcType == "i64" && tgtType != "i128" && toLLVMType(tgtType) != "i64" {
...
func (g *Generator) coerceToInt(sb *strings.Builder, v string, exprForType parser.Expression, targetType string) string {
	...
	if strings.HasPrefix(v, "%") {
		...
		cvtReg := g.tmpReg("cvt")
		if srcType == "i64" && toLLVMType(targetType) != "i64" {
			// i64 → 較窄型別：trunc
		...
		if fieldIdx >= 0 && sb != nil {
			...
			if fieldType == "%vec" {
				...
				if strings.HasPrefix(val, "%") {
					...
					if valType != "" && toLLVMType(valType) != toLLVMType(vecElemType) && g.isIntegerLLVMType(valType) && g.isIntegerLLVMType(vecElemType) {
				...
				// GEP to element index
				elemGEP := g.tmpReg("set.vec.elem")
				sb.WriteString(fmt.Sprintf("%s%s = getelementptr inbounds %s, %s* %s, i64 %s\n",
					g.indent(), elemGEP, vecIRType, vecIRType, dataTyped, idx))
				// 若元素是堆擁有型別（如 %str-long），需先 free 舊元素再深層 clone，
				// 避免淺拷貝 data 指標導致 double-free / use-after-free。
				if g.isHeapOwningType(vecElemType) {
					// free 舊元素的堆數據
					g.emitElementFree(sb, elemGEP, vecElemType)
					// 將 RHS 值存入臨時 alloca，再深層 clone 到目標位置
					tmpAlloca := g.tmpReg("set.vec.srctmp")
			...
			if strings.HasPrefix(fieldType, "[") {
				...
				if closeB > 0 {
					...
					if elemType != "i64" && !g.isStructLLVMType(elemType) && strings.HasPrefix(val, "%") {
						...
						if toLLVMType(valType) != toLLVMType(elemType) {
						...
						if intLit, ok := value.(*parser.IntegerLiteral); ok && intLit.Value == 0 {
							...
							if !isStrLit && !isIdent && !isCall {
								...
								if isIdx || g.isStringExpr(value) {
									// IndexExpression for vec/array struct elements returns a
									// loaded value (tracked in ssaTypes) — skip the load.
									// Slice path and struct array field path return pointers — load needed.
									alreadyLoaded := false
									if g.ssaTypes != nil {
										if ssaType, ok := g.ssaTypes[val]; ok && g.isStructLLVMType(ssaType) {
											alreadyLoaded = true
										}
									}
									if !alreadyLoaded {
...
func (g *Generator) generateNestedStrIndexAssign(sb *strings.Builder, innerIdx *parser.IndexExpression, index parser.Expression, value parser.Expression) string {
	...
	if sb != nil && g.outputParamNames != nil {
		if dot, ok := innerIdx.Left.(*parser.DotExpression); ok {
			if ident, ok := dot.Receiver.(*parser.Identifier); ok {
				if g.outputParamNames[ident.Value] {
					g.emitSetRetInitBit(sb, ident.Value)
				}
			}
		}
	}
		...
		if fields, ok := g.structTypes[structName]; ok {
			...
			if fieldIdx >= 0 && sb != nil {
				...
				if g.isStructLLVMType(fieldType) {
					...
					if intLit, ok := expr.Value.(*parser.IntegerLiteral); ok && intLit.Value == 0 {
						...
						if !isStrLit && !isIdent {
							...
							//   - IndexExpression when it returns a GEP pointer (slice path,
							//     struct array field path)
							//   - InfixExpression (string concat/repeat → alloca pointer)
							// CallExpression and DotExpression return loaded values, not pointers.
							// IndexExpression for vec/array struct elements also returns a
							// loaded value (tracked in ssaTypes) — skip the load in that case.
				...
				if storeVal == "zeroinitializer" || !g.isHeapOwningType(fieldType) {
					...
					sb.WriteString(fmt.Sprintf("%sstore %s %s, %s* %s\n",
						g.indent(), toLLVMType(fieldType), storeVal, toLLVMType(fieldType), cloneSrc))
		...
		if strings.HasPrefix(t, "[") {
			...
			if strings.HasSuffix(t, "*") {
				...
				if elemType != "" {
					...
					// Track SSA type for struct elements so downstream consumers
					// (e.g. struct field assignment) know this is a loaded value, not a pointer.
					if g.ssaTypes != nil && g.isStructLLVMType(llvmElemType) {
		...
		if ident, ok := argExpr.(*parser.Identifier); ok && sb != nil {
			if varType, hasType := g.varTypes[ident.Value]; hasType && g.isHeapOwningType(varType) {
				...
				if g.elemElemTypes != nil {
					elemElemType = g.elemElemTypes[ident.Value]
				}
				...
				g.emitDeepClone(sb, g.varAddr(ident.Value), cloneBuf, varType, elemType, elemElemType)
				argTypeStr = varType + "*"
				argPtr = cloneBuf
			}
		}
