...
通過將 MD5 實現從 `i64` + `& mask` 改為原生 `u32` 類型，獲得了顯著的效能提升：

| 指標 | i64 + &mask (舊) | u32 原生 (新) | 變化 |
|------|------------------|---------------|------|
| 耗時（最佳） | 26.2ms | 22.5ms | **-14.1%** |
| 周期數 | 71M | 55M | **-22.5%** |
| 指令數 | 197M | 196M | -0.5% |
| IPC | 2.77 | 3.57 | **+28.9%** |
| 與 Rust 差距 | 1.66× | 1.50× | **-9.6%** |
| `& mask` 指令 | 128/區塊 | 0 | **-100%** |

**關鍵發現**：指令數幾乎不變（-0.5%），但周期數大幅下降（-22.5%）。這是因為 `& mask` 操作雖然指令數不多，但會延長依賴鏈——每次 `add` 後必須等待 `and` 完成才能使用結果。改用 `u32` 後，`add i32` 自然溢出，無需後續 `and`，依賴鏈縮短，指令級並行度（IPC）從 2.77 提升至 3.57。
...
|------|----------|------------|------|
| **整數寬度** | `u32` 原生 32 位，`wrapping_add` 自動溢出 | `u32` 原生 32 位，`i32` 自然溢出 | **已消除** ✅ |
...
#### 優化 5：使用 u32 原生類型（消除所有 & mask）
- 將 `mask`、`h0-h3`、`a-d`、`f`、`t`、`w0-w15` 全部聲明為 `u32`
- LLVM `i32` 算術自然 32 位溢出，無需 `& mask`
- IR：`and i64 ... 4294967295` 從 128 次/區塊降至 0
- 周期數從 71M 降至 55M（-22.5%），時間從 26.2ms 降至 22.5ms
- IPC 從 2.77 提升至 3.57（+28.9%）
- 編譯器已有的 `u32` → `i32` 映射基礎設施（`intExprLLVMType`、`arithLLVMType`、`coerceToInt`）自動處理型別推導和轉換
...
1. ~~**支援 `u32` 類型**：消除所有 `& mask` 操作~~ ✅ 已完成（周期數 -22.5%）
