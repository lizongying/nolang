...
|------|----------|--------|--------|-----|----------|------------|
| **Rust** | 16.1 | 80M | 34M | 2.34 | 1.5MB | 464K |
| **Nolang** | 17.3 | 89M | 39M | 2.28 | 1.5MB | 757K |
...
- **Rust** 最快（16.1ms），md-5 crate 使用原生 `u32` 類型和 `u32::from_le_bytes` 零開銷載入
- **Nolang** 次之（17.3ms），為 Rust 的 **1.07×**，比 Go 快 **3.8×**
- **Go** 最慢（65.5ms），crypto/md5 標準庫在此場景下效能較低
...
| **range 迴圈開銷** | 無（Rust 用 `for` + 迭代器） | 方向選擇分支 (`rng.dircmp`/`rng.sel`) | ~1M 指令 |

**剩餘差距**（~1.07×）：主要來自：
1. **末塊殘留邊界檢查**（7 次/呼叫 × 100K = 700K 次）：末塊的剩餘位元組拷貝仍有逐字節邊界檢查
2. **range 迴圈方向選擇開銷**：Nolang 的 `bidx <- [0..N)` 語法生成 `rng.dircmp`/`rng.asc`/`rng.desc`/`rng.sel` 分支，每次迴圈約 4-5 條額外指令
3. **memset 開銷**：每次 md5 呼叫都需 memset 64 位元組緩衝區
4. **LLVM 優化路徑差異**：Rust IR 使用 `noalias`/`dereferenceable` 等屬性幫助 LLVM 優化，Nolang IR 缺少這些屬性
...
| 指標 | i64 優化前 | u32 優化後 | 內建函數+alloca 後 | 直接載入優化後 | 總變化 |
|------|-----------|-----------|-------------------|----------------|--------|
| 耗時 | 28.0ms | 22.5ms | 18.4ms | 17.3ms | **-38.2%** |
| 指令數 | 203M | 196M | 94M | 89M | **-56.2%** |
| 周期數 | 80M | 55M | 38M | 39M | **-51.3%** |
| 峰值 RSS | 7.6MB | 7.6MB | 1.5MB | 1.5MB | **-80.3%** |
| 與 Rust 差距 | 1.77× | 1.50× | 1.15× | 1.07× | **-38.4%** |
