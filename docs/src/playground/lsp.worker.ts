...
function runBatch(messages: object[]): JsonRpcResponse[] {
  ...
  // Use an explicit in-memory filesystem with `/` preopened against
  // it. `preopens: {'/': '/'}` without `fs` would attempt to bind the
  // host root (absent in the browser) and fail. With `fs: MemFS`,
  // wasmer preopens `/` inside the MemFS, granting the LSP server a
  // writable root. Empty `preopens: {}` is insufficient because WASI
  // requires paths to be preopened before the guest can access them.
  const fs = new MemFS();
  const wasi = new WASI({
    args: ['lsp'],
    env: {},
