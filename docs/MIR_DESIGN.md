# Nolang MIR — 工业级中端中间表示设计（实施记录 v2.15）

> 状态：直接发射已达 **Stage 3**（MIR→LLVM 直接发射，全量 corpus 门禁可跑；**第三十二轮（2026-09-15）权威全量扫描：`MATCH=368/421`（≈87.4%）、`DIVERGE=1`、`MIR 专属 gap=0`、`两模式都失败=51`（CERR 39 + CRASH 12）、`HANG=1`（`test-for2.no`，两模式都挂死）**。本轮修 **#64**（`emitIndex` 读取 `%str-long` 元素时未做 deep clone → 共享堆缓冲区 → drop 后悬空指针 → `test-diff-debug.no` 的 DP 表恒全 0 / segfault），闭环唯一稳定 MIR gap。**第三十一轮的 `MATCH=367`/`CRASH=14` 与第三十二轮的 `MATCH=368`/`CRASH=12` 之差：+1 MATCH（`test-diff-debug.no` 从 CRASH→MATCH）、−2 CRASH（`test-diff-debug.no` + `test_fs_error_complete.no` 从 CRASH 桶转出，后者变为 DIVERGE——fs.open 对目录的行为差异，非 MIR 回归）**。详见 §13.3.10）
> 作者：编译器工作流
