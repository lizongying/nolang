...
71. **Parser 层面 self 从 Parameters 移到 Results，完成 out-param 全链路对齐（2026-09-18 第四十八轮）**：#70 在 `tohir.go` 的 `funcLike` 中做了 self 从 KParam→KResult 的 HIR 层转换，但 parser 的 `decl.go` 仍把 self 插入 `Parameters`（输入参数），`tohir.go` 再从 Parameters 中提取并转为 KResult——这是一个 workaround，正确做法是在 parser 声明阶段就直接把 self 放到 `Results`（输出参数）中。**修法**：① `decl.go` 三处（`parseMethodDefinition`、`parseArrayTypeMethodDefinition`、`parseColonMethodDefinition`）把 `funcDef.Parameters = append([]*Parameter{selfParam}, ...)` 改为 `funcDef.Results = append([]*Parameter{selfParam}, ...)`；② `tohir.go` 的 `funcLike` 改为从 `sig.Results[0]` 检测 self（而非 `sig.Parameters[0]`），不再需要从 Parameters 中提取；③ `checker.go` 5 处 `fd.Parameters[0].Name == "self"` 改为 `fd.Results[0].Name == "self"`；④ `funcSigFromDef` 排除方法定义中的 self 不计入 `ResultTypes`（避免返回值计数多1）；⑤ `build/module_prefix.go` 同步改为从 `Results[0]` 查找 self；⑥ `fmt/stmt.go` 格式化时跳过 Results 中的 self（而非 Parameters 中的）；⑦ `build/transpiler.go` 的 `validateStmtArrayBounds` 中 Results 的 slice 类型检查也跳过 self（与 Parameters 的逻辑对齐）；⑧ 测试文件 `interface_test.go` 和 `generic_structs_test.go` 同步更新。验证：`no vet src/std` ERROR 数 377→377（无新增）；`parser`/`mir`/`build` Go 测试全绿；`test-basic.no`/`test-arr.no`/`test-vec.no`/`test-str.no`/`test-all.no`/`test-cmp.no` 全部 PASS；预先存在的 `checker`/`fmt` 测试失败未变化。
