# Nolang MIR — 工业级中端中间表示设计（实施记录 v2.7）
...
- 已闭环的 **43 个** MIR 专属运行时崩溃/代码生成修复目录（#1–#43，早期含 §13.3.4 顶层 main 合成结构修复 #17、`emitBitwise` option 解包包回 #18、`print` 内联 option payload 路由 #19；近期含 #19 print payload、#24–#27 mem-safety 崩溃族、#28 字符串插值、#29–#35 切片/窄整型/类型推导/内建补齐、#36 顶层未初始化全局、#37/#38 net FFI 内置、#39 `await`/`run` task 运行时、#40 async 取消/让出内置、**#41 C 家族 red-line 3 崩溃全闭环（语法化 `run` + 异步实参强制 + 任务结果类型跟踪 + `#{embed}` 物化）**；**#42 值类型别名展开（`fd=i64` 等 newtype receiver 经 `Module.ValueTypeAliases`+`expandTypeAlias` 在 `resolveCallee` 展开为底层型别，闭环 `test_errno_basic` 的 `fd.to-str`→`i64.to-str`）**；**#43 跨模块自由函数调用名解析（`# /path` 模块自由函数注册 bare 名、`resolveModuleCallName` 优先 qualified 否则回退 bare，闭环 `mem-safety/bug13-bool-coercion` 的 `helper.compute-str`→`compute-str`）**）；
- 当前已知的 gap 家族：**第二十轮（2026-09-13 续）全量重扫确认 MIR 专属 gap 仍为 1、DIRVERGE 由 51 降至 43**；**第二十三轮（2026-09-14）全量重扫 MIR_GAP 分类 0（test-diff-debug 被 legacy flake 重分类进 LEGACY_FAIL 的假象，稳定缺口仍属 interp 族）、DIRVERGE 再降至 33**（`./bin/no`，421 文件，MATCH=283/DIVERGE=33/MIR_GAP=0/LEGACY_FAIL=96/HANG=9）；第十九轮确认 match/if 作表达式捕获机制经「可变 slot + 每 arm `EmitMoveInto` + KIdent 变体块先查 locals 再发零（防 red-line trap）」正确实现后，闭环 8 测试（`test-if-str`/`test-match-basic`/`test-minimal-it`/`test-minimal-it2`/`test-minimal-ok`/`test-opt-match-all`/`test-opt-match2`/`test-opt-match3`），并消除 3 个 `match 42:{err->}` 类非 option subject 的 `trace/BPT trap`（→DIVERGE，rc=0 安全）。第十六轮 3 缺口（`test-open-read`/`test-fs-struct`/`test-str`）均已于第十六轮闭环（`it` 类型传播 + match 臂合成 `let it` 改 ALIAS）；第十七轮闭环 `?bool` 打印族 3 测试；第十八轮闭环 `test-range.no`（双向 range-for）；前述 `bug13-bool-coercion`/`test-bare-match`/`test-ok-shadow` 在第十九/二十轮均保持 MATCH。**仅剩 1 个稳定 MIR_GAP**：`test-diff-debug.no`（interp 族 + legacy flaky，非 codegen 缺口）。详见 **§13.3.7**（含对早期"gap=0"误判及第十四轮"17"逐文件清单不准确的纠错）；
...
| 类别 | 数量（**第二十轮 2026-09-13 续 权威全量重扫（`./bin/no`，全量递归 `tests/**/*.no` = 421，25s/测超时，8-worker）**：**MATCH=279 DIVERGE=43 MIR_GAP=1 LEGACY_FAIL=97 HANG=1**；**第二十三轮（2026-09-14）两次重扫：round-24 MATCH=283/DIVERGE=33/MIR_GAP=0/LEGACY_FAIL=96/HANG=9；round-25 MATCH=285/DIVERGE=37/MIR_GAP=1/LEGACY_FAIL=97/HANG=1（稳定 MIR_GAP=1、HANG≈1，round-24 的 9 系争用假象）**。第十九轮为 271/51/1/97/1；第十八轮为 271/51/1/97/1；第十七轮 269/52/1/98/1；第十六轮 267/55/1/97/1；第十五轮为 261/55/3/98/4（两次一致）；第十四轮 248/58/17/97/1（"17"逐文件清单已证不准确，真实稳定 gap=3）；第十三轮 240/57/21/98/4。此前 "323 PASS / MATCH 322 / gap 0" 系**定向子集**口径，见 §13.3.7 纠错） | 说明 |
|---|---|---|
...
| **MIR 专属 gap**（legacy 过、MIR=3 挂） | **1**（**第二十轮权威全量重扫实测**，2026-09-13 续；**第二十三轮分类为 0，但系 `test-diff-debug` 被 legacy flake 重分类进 LEGACY_FAIL 的假象，稳定缺口仍为 1**；第十九轮 271/51/1/97/1；第十八轮 DIVERGE 52→51（`test-range` 双向 range-for 闭环）；第十七轮 DIVERGE 55→52（`?bool` 打印族 3 测试闭环）；第十六轮 3 → −2（match 臂 `it` 类型传播 + ALIAS 闭环 `test-open-read`/`test-fs-struct`/`test-str`）；**第二十轮 DIVERGE 51→43（match/if 作表达式 8 测试闭环 + 消除 3 个非 option-subject `trace/BPT trap`）**；**仅剩 `test-diff-debug.no` 1 个稳定 MIR_GAP**（legacy `MIR=0` rc 在 1/0 间 flaky 抖动，属 interp 族，非 codegen 缺口）） | 真正的 MIR codegen 缺口收敛至 **1**（§13.3.7）：`test-diff-debug.no`（`unsupported construct (string interpolation)` 族，legacy flaky）。**C 家族 3 个 red-line 运行时崩溃已于第十四轮全部闭环**（#41）；`test_errno_basic`(`fd.to-str`)、`bug13-bool-coercion`(`helper.compute-str`) 两 `unknown callee` 已于第十五轮闭环（#42/#43）；`test-open-read`/`test-fs-struct`/`test-str` 三 match-臂 `it` 绑定缺陷已于第十六轮闭环；`?bool` 打印族 DIVERGE 已于第十七轮闭环；`test-range` 降序 range DIVERGE 已于第十八轮闭环；match/if 作表达式 8 测试 DIVERGE 已于第二十轮闭环（详见 §13.3.7 各轮更新）。此前 `test-async.no`/`test-i8-debug2.no` 等归入 "MIR 正确 / legacy 错误" 族，不在 gap 集内 |
