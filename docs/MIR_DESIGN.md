# Nolang MIR — 工业级中端中间表示设计（实施记录 v2.17）

> 状态：直接发射已达 **Stage 3**（MIR→LLVM 直接发射，全量 corpus 门禁可跑；**第三十四轮（2026-09-15）权威全量扫描：`MATCH=372/421`（≈88.4%）、`DIVERGE=0`、`MIR 专属 gap=0`、`两模式都失败=48`（CERR 34 + CRASH 14）、`HANG=1`（`test-for2.no`，两模式都挂死）**。本轮修 **#67**（标量→`str` 参数自动类型转换：`emitCall` 的 owned 参数传递路径中，当实际参数类型为 `i64`/`double`/`i1` 而参数声明类型为 `%str-long` 时，自动插入 `@str_from_i64`/`@str_from_double`/`@str_from_const("true"/"false")` 转换调用，而非直接 `store %str-long %i64_val, %str-long* %slot` 导致 LLVM 类型不匹配；新增 `@str_from_double` 运行时 helper，镜像 `print_double` 的 `%g` 格式化逻辑但写入 `%str-long` 而非直接 stdout；同时新增 `NOLANG_MIR_KEEP_IR` 环境变量保留 opt 验证临时 IR 文件以便调试）。**第三十三轮的 `MATCH=369`/`DIVERGE=0`/`CERR=37`/`CRASH=14` 与第三十四轮的 `MATCH=372`/`DIVERGE=0`/`CERR=34`/`CRASH=14` 之差：+3 MATCH、−3 CERR（`test-option-index.no` 从 CERR→MATCH，`i64` 被传给 `str` 参数的 `store %str-long %i64` 类型不匹配已修；`test-iout-autoconvert.no` 从 CERR→MATCH，`double`/`bool` 被传给 `str` 参数的同类问题已修；第 3 个测试从 CERR→MATCH，同类 `i64 vs %str-long` 类型不匹配已修）**。详见 §13.3.11）
> 作者：编译器工作流
