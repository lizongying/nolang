# Nolang MIR — 工业级中端中间表示设计（实施记录 v2.11）
...
- 当前已知的 gap 家族：**第二十八轮（2026-09-15）全量并行扫描确认 MIR 专属 gap = 0**（全量递归 420 文件，MATCH=351/DIVERGE=1（假阳性，MIR 成功而 legacy 失败）/CERR=55（全部 legacy 也失败）/CRASH=13（全部 legacy 也失败）/HANG=0）；**第二十七轮（2026-09-14）全量并行扫描 MATCH=351/DIVERGE=1/MIR_GAP=0**（420 文件，CERR=62/CRASH=6）；**仅剩 0 个稳定 MIR_GAP**。详见 **§13.3.7**（含对早期"gap=0"误判及第十四轮"17"逐文件清单不准确的纠错）；
