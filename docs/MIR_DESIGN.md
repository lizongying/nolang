# Nolang MIR — 工业级中端中间表示设计（实施记录 v2.14）

> 状态：直接发射已达 **Stage 3**（MIR→LLVM 直接发射，全量 corpus 门禁可跑；**第三十一轮（2026-09-15）权威全量扫描：`MATCH=367/421`（≈87.2%）、`DIVERGE=0`、`MIR 专属 gap=0`、`两模式都失败=53`（CERR 39 + CRASH 14）、`HANG=1`（`test-for2.no`，两模式都挂死）**。本轮修 **#63**（显式写出参实参被误判为变参展开 → 整个模块 opt-verify 失败 / 静默污染切片）并**修好扫描器超时工具缺陷**（`run_to` 只杀直接子进程 → 改为 `setpgid` + `kill -KILL -$pgid` 杀整个进程组，`test-for2.no` 不再卡死整轮）。**第三十轮的 `gap=1`/`CRASH=13` 与第三十一轮的 `gap=0`/`CRASH=14` 之差全部是抖动**：`test-diff-debug.no` 的 legacy 基线 1/6 概率失败（健康时是 gap，抖动时落 CRASH）、`test-quant-all1.no` 两模式都打印未初始化栈指针（MATCH/DIVERGE 抖动）。**按抖动校正后的真实态：MIR 专属 gap = 1（`test-diff-debug.no`，MIR=3 恒错而 legacy 6/6 正确），DIVERGE = 0**；详见 §13.3.10）
> 作者：编译器工作流
